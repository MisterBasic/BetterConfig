...
public class ConfigParser {
		...
		while(matcher.find()) {
			...
			for(Token t : Token.values()) {
				if(data.matches(t.regex())) {
					if(t.equals(Token.COMMENT)) { // Ignore comments
						break;
					}
	...
	public void parse() {
		...
		for(int i = 0; i < tokens.length + 1; i++) {
			if(i >= tokens.length)
				token = new Tok(Token.ENDL, ";", -1); // Fake a token for terminator
			else
				token = tokens[i];
			...
			if(readValue) {
				...
				case SECTION: {
					...
					debug("[Parser] Section \"" + currentSection.getName() + "\" defined.");
		...
		if(currentSection != null) {
			this.sections.add(currentSection);
			debug("[Parser] Section \"" + currentSection.getName() + "\" defined. (EOF-terminated)");
		}
		long end = System.currentTimeMillis();
	...
	private ConfigProperty evaluate(String propName, Stack<Tok> tokens) {
		if(tokens.size() < 1) {
			return null;
		}
	...
	private boolean isInvalidValue(Stack<Tok> value) {
	...
	private ConfigArray evaluateArray(String propName, Stack<Tok> tokens) {
		int nestingLevel = 0;
